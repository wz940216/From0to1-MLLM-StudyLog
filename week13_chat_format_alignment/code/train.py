...
def build_train_dataset(train_config, max_samples=None):
    """根据 DATA.TRAIN_DATASET 构造单任务或多任务训练集。"""
...
def main():
    ...
    parser.add_argument("--config", default="week13_chat_format_alignment/code/config.yaml", help="训练配置文件路径")
    ...
    dataset = build_train_dataset(train_config, max_samples=args.max_samples)
    ...
    log_dir = config["TRAINING"]["LOGGING"].get("LOG_DIR", "week13_chat_format_alignment/outputs/logs")
