...
也可以加入我的知识星球。这个星球会围绕「CV 到多模态实战 24 周」持续更新，我会把学习路线、代码实践、踩坑记录、模型训练细节和阶段性总结整理成更系统、更完整的笔记。

相比零散的文章，知识星球里的内容会更偏向体系化沉淀：每一周都会尽量围绕一个明确主题展开，从基础概念、论文/源码理解，到可运行的工程实践，再到实验复盘，帮助大家把 CV、多模态、MiniLLaVA 这类项目真正串起来。

如果你希望有人一起推进学习进度，交流实现细节，或者想参考一套持续更新的实战路线，也欢迎加入，一起把这 24 周认真走完。

后续星球中也会持续更新更多多模态落地实战内容，包括真实项目案例拆解、多模态大模型训练与微调实践、以及从 Demo 到可用产品过程中会遇到的工程问题。

除了 CV 到多模态实战 24 周主线，后续也会考虑补充一些更贴近业务场景的专题，比如：多模态大模型微调、视觉问答 VQA、图像描述生成、OCR + LLM 文档理解、表格/票据/合同解析、RAG 与多模态检索、工业质检、模型压缩与部署、推理加速、服务化上线等。总之，我在学什么、做什么、踩过什么坑，就尽量把它整理出来。希望把这些真实经历沉淀成有意义的内容，帮助大家既能理解背后的原理，也能真正动手复现、改造，并逐步迁移到自己的项目中。
