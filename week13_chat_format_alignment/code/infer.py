...
def build_context_prompt(history, question, system_prompt=None):
...
def answer_one_turn(model, image, history, question, gen_config, system_prompt=None):
    """执行一轮带上下文的图文对话推理，并返回 assistant 回答。"""
    prompt = build_context_prompt(history, question, system_prompt=system_prompt)
...
def main():
    ...
    parser.add_argument("--config", default="week13_chat_format_alignment/code/config.yaml")
    parser.add_argument(
        "--checkpoint",
        default="week13_chat_format_alignment/outputs/checkpoints/step_2109.pt",
    ...
    if args.interactive:
        ...
        while True:
            ...
            answer = answer_one_turn(model, image, history, question, gen_config, system_prompt=system_prompt)
            history.extend([
