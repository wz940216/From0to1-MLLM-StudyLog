...
def answer_one_turn(model, image, history, question, gen_config, system_prompt=None, max_history_turns=None, retry_on_json_error=True):
    ...
    input_ok, input_hit_word = input_filter(question)
    if not input_ok:
        return SAFE_REFUSAL, {
            "blocked": True,
            "stage": "input",
            "hit_word": input_hit_word,
        }
    ...
    if parsed.ok:
        filtered_answer, output_hit_word = output_filter(parsed.text)
        return filtered_answer, {
            "blocked": output_hit_word is not None,
            "stage": "output" if output_hit_word else None,
            "hit_word": output_hit_word,
        }
    if not retry_on_json_error:
        filtered_answer, output_hit_word = output_filter(answer)
        return filtered_answer, {
            "blocked": output_hit_word is not None,
            "stage": "output" if output_hit_word else None,
            "hit_word": output_hit_word,
    ...
    final_answer = retry_parsed.text if retry_parsed.ok else retry_answer
    filtered_answer, output_hit_word = output_filter(final_answer)
    return filtered_answer, {
        "blocked": output_hit_word is not None,
        "stage": "output" if output_hit_word else None,
        "hit_word": output_hit_word,
...
def main():
    ...
    if args.interactive:
        ...
        while True:
            ...
            answer, safety = answer_one_turn(model, image, history, question, gen_config, system_prompt=system_prompt, max_history_turns=args.max_history_turns)
            history.extend([
