...
class Projector(nn.Module):
    ...
    def __init__(self, input_dim=768, hidden_dim=2048, output_dim=2048, freeze=False):
        super().__init__()
...
class LLMDecoder(torch.nn.Module):
    def __init__(self, model_path, r=8, lora_alpha=32, lora_dropout=0.1, freeze=False, device="cuda"):
        ...
        # 给 tokenizer 注册 <image> special token
        ...
        if added_tokens > 0:
            # 这样 <image> 不会被 tokenizer 当普通字符串拆开。
            self.model.resize_token_embeddings(len(self.tokenizer))

        use_lora = r is not None and str(r).lower() != "none"
        if use_lora:
            r = int(r)
            # LoRA 参数配置
