...
class LlavaPretrainDataset(Dataset):
    ...
    def __init__(self, dataset_path, image_dir, annotation_file, max_samples=None,
                 task_name=None, split="train", train_split=1.0, split_seed=42):
        ...
        self.samples = self._select_split(self.samples, split, train_split, split_seed)
        if max_samples is not None:
            self.samples = self.samples[:max_samples]

    def _select_split(self, samples, split, train_split, split_seed):
        if split not in {"train", "val"}:
            raise ValueError(f"split 必须是 train 或 val，当前是: {split}")

        train_split = float(train_split)
        if not 0 < train_split <= 1:
            raise ValueError(f"TRAIN_SPLIT 必须在 (0, 1] 范围内，当前是: {train_split}")
        if train_split >= 1:
        ...
        selected_indices = indices[:split_index] if split == "train" else indices[split_index:]
